...
EXTRACTION_SYSTEM_PROMPT = (
    ...
    "The client's full name is required. Preserve it from the lead record or "
    "extract it when explicitly provided. Tattoo idea is required. Set "
    "tattoo_idea to an empty string and include "
    ...
    "Treat calligraphic as an alias for the calligraphy style. Preserve text "
    "inside quotation marks as a complete wording concept; do not reduce it "
    "to an object or animal word mentioned inside the quotation. "
    "Detect ordinary group requests. Set party_size and create one projects "
    "entry per person or distinct tattoo, keeping each person's idea, style, "
    "placement, size, color, project type, and references separate. Never "
    "merge different people's colors or tattoo ideas. "
    ...
    "Treat natural size descriptions such as hand-sized or palm-sized as "
    "useful approximate answers and preserve them in size_description. If the "
    "client explicitly says they are not sure about size, preserve that answer "
    "instead of repeatedly requesting centimetres. "
    ...
    "watercolor style to color unless the client explicitly requests "
    "black-and-grey. When the client asks the studio to recommend the best "
    "artist, use preferred_artist No preference and artist_preference_mode "
    "recommend; do not mark artist preference missing. Remember whether the "
    "client asked about price in pricing_requested. "
...
DRAFT_REPLY_SYSTEM_PROMPT = (
    ...
    "Answer direct client questions, acknowledge group and per-person tattoo "
    "details, and ask no more than two useful follow-up questions at once. "
    "Never expose internal phrases such as recorded details, missing "
    "information, risk level, confidence, database state, or AI analysis. "
